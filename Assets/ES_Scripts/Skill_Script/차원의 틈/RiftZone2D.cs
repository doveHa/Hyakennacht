...
public class RiftZone2D : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            var col = _buf[i];
            if (!col) continue;
            if (!PassesFilter(col)) continue; // ← 시그니처 바뀜 (아래 참고)
            ...
            {
                Vector2 dir = to / dist;
                float arriveRadius = _radius * 0.7f;                  // 감속 시작 지점
                ...
                // 힘 제한(너무 큰 가속 방지)
                float maxAccel = _pullForce; // ForceMode2D.Force 기준
    ...
    IEnumerator CoDamageTicks()
    {
        ...
        while (_armed && Time.time < _endTime)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                var col = _buf[i];
                if (!col) continue;
                if (!PassesFilter(col)) continue; // ← 변경
    ...
    void Finish()
    {
        ...
        else gameObject.SetActive(false); // (백업)
    }
